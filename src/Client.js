...
class Client extends EventEmitter {
    ...
    async attachEventListeners() {
        ...
        await this.pupPage.evaluate(() => {
            if (!window.Store) {
                throw new Error('Store not available for event listener attachment');
            }
            const S = window.Store;
            if (S.Msg) {
                S.Msg.on('change', (msg) => { window.onChangeMessageEvent(window.WWebJS.getMessageModel(msg)); });
                S.Msg.on('change:type', (msg) => { window.onChangeMessageTypeEvent(window.WWebJS.getMessageModel(msg)); });
                S.Msg.on('change:ack', (msg, ack) => { window.onMessageAckEvent(window.WWebJS.getMessageModel(msg), ack); });
                S.Msg.on('change:isUnsentMedia', (msg, unsent) => { if (msg.id.fromMe && !unsent) window.onMessageMediaUploadedEvent(window.WWebJS.getMessageModel(msg)); });
                S.Msg.on('remove', (msg) => { if (msg.isNewMsg) window.onRemoveMessageEvent(window.WWebJS.getMessageModel(msg)); });
                S.Msg.on('change:body change:caption', (msg, newBody, prevBody) => { window.onEditMessageEvent(window.WWebJS.getMessageModel(msg), newBody, prevBody); });
                S.Msg.on('add', (msg) => {
                    if (msg.isNewMsg) {
            ...
            if (S.AppState) S.AppState.on('change:state', (_AppState, state) => { window.onAppStateChangedEvent(state); });
            if (S.Conn) S.Conn.on('change:battery', (state) => { window.onBatteryStateChangedEvent(state); });
            if (S.Call) S.Call.on('add', (call) => { window.onIncomingCall(call); });
            if (S.Chat) {
                S.Chat.on('remove', async (chat) => { window.onRemoveChatEvent(await window.WWebJS.getChatModel(chat)); });
                S.Chat.on('change:archive', async (chat, currState, prevState) => { window.onArchiveChatEvent(await window.WWebJS.getChatModel(chat), currState, prevState); });
